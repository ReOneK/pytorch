...
def test(**kwargs):
    ...
    if opt.load_model_path_test:
        model.load_state_dict(t.load(opt.load_model_path_test))
    model.to(opt.device)
...
def train(**kwargs):
    ...
    if opt.load_model_path_train:
        model.load(opt.load_model_path_train)
    model.to(opt.device)
    ...
    for epoch in range(opt.max_epoch):
        ...
        t.save(model.state_dict(), './checkpoints/squeezenet{epoch}.pth'.format(epoch=epoch))
